...
const CostCalculator = {
        ...
        jQuery.ajax({
            ...
            success: function(response) {
                if (response.success && response.data) {
                    console.log('💰 Dynamic pricing loaded:', response.data);
                    self.processPricingData(response.data);
                    self.pricingLoaded = true;
                    self.updateEstimatedCost();
                } else {
                    console.error('Failed to load pricing data:', response);
    ...
    processPricingData: function(data) {
        ...
        if (data.openai) {
            for (const [model, pricing] of Object.entries(data.openai)) {
                this.modelPricing[model] = {
                    input: pricing.input / 1000,  // Convert from per 1M to per 1K tokens
        ...
        console.log('💰 Processed pricing data:', this.modelPricing);
    },
    ...
    fallbackToPricing: function() {
        ...
        console.log('💰 Fallback pricing loaded:', this.modelPricing);
    ...
    getCurrentModel: function() {
        ...
        if (modelSelect.length) {
            ...
            console.log('💰 Initial model from form:', this.currentModel);
            const self = this;
            modelSelect.on('change', function() {
                self.currentModel = jQuery(this).val();
                console.log('💰 Model changed to:', self.currentModel);
                self.updateEstimatedCost();
            });
        } else {
            ...
            if (typeof ai_scribe !== 'undefined' && ai_scribe.aiEngine && ai_scribe.aiEngine.model) {
                ...
                console.log('💰 Model from ai_scribe settings:', this.currentModel);
            } else {
                console.log('💰 No model found, using default gpt-4o');
    ...
    updateEstimatedCost: function() {
        console.log('💰 updateEstimatedCost called - pricingLoaded:', this.pricingLoaded, 'currentModel:', this.currentModel);

        if (!this.pricingLoaded) {
            console.log('💰 Pricing not loaded yet, skipping cost update');
            return;
        }

        const pricing = this.modelPricing[this.currentModel];
        console.log('💰 Pricing for model', this.currentModel, ':', pricing);
        console.log('💰 Available models in pricing:', Object.keys(this.modelPricing));
        ...
        if (pricing) {
            ...
            if (pricing.article_estimate) {
                ...
                console.log('💰 Using article estimate:', totalEstimated);
            } else {
            ...
            console.warn('💰 No pricing data for model:', this.currentModel);
            console.warn('💰 Available pricing models:', Object.keys(this.modelPricing));
    ...
    addActualCost: function(inputTokens, outputTokens, model) {
        ...
        if (pricing && inputTokens && outputTokens) {
            ...
            console.warn('💰 Cannot calculate cost - missing data:', {
                pricing: !!pricing,
                inputTokens: inputTokens,
                outputTokens: outputTokens,
                model: usedModel
...
jQuery( document ).ready( function () {
	...
	jQuery(document).on('change', 'select[name="model"]', function() {
		const newModel = jQuery(this).val();
		console.log('💰 Model changed in settings to:', newModel);
		CostCalculator.currentModel = newModel;
		CostCalculator.updateEstimatedCost();
	});
		...
		jQuery.ajax( {
			...
			success: function ( response ) {
				...
				if (actualResponse.usage || actualResponse.token_usage) {
					...
					console.log('💰 Cost Tracking - Input tokens:', inputTokens, 'Output tokens:', outputTokens, 'Model:', model);
