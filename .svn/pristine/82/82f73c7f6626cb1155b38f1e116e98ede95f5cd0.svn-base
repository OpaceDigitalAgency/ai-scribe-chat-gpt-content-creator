...
class AI_Scribe {
	public function __construct() {
		...
		// Add DALL·E 2 image generation action
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( ! empty( $isError ) ) {
				...
				if ($actionInput == 'article') {
				    ...
				    // Generate image HTML
				    $image_html = '<img src="' . esc_url($attachment_url) . '" alt="' . esc_attr($truncated_title) . '" />';
