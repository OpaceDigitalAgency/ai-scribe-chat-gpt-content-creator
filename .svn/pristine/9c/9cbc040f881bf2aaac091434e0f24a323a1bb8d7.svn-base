 ...
 * Plugin Name: 		AI Scribe: AI SEO Content Creator, Blog Writer, Humanizer & SEO Assistant ChatGPT (OpenAI GPT-4o 128K) 
 * Description: 		AI Scribe: Free Plugin, ChatGPT, AI, SEO, Content Creator & Writer (Blog Writer), Keyword Research, Title Suggestions, Editable Prompts, OpenAI, GPT, Auto Article Writer, SEO Analysis, Article Evaluation, GPT-4o (128K), and GPT-4o-mini (128K). Compatible with Yoast SEO, Rank Math, AIOSEO & SEOPress. Free Plugin - No hidden costs or paid add-ons.  
 * Plugin URI: 			https://www.opace.co.uk
 * Text Domain: 		ai-scribe-gpt-article-builder
 * Tags: 				AI, ChatGPT, SEO, Content Creator, Content Writer, Blog Writer, Content Generator, Content Writer, Blog Writer, OpenAI, GPT-4o, GPT-4o-mini, 128K, Text Creator, Blog Creator, Blog Builder, Article Writer, Content Marketing, Free, GPT, OpenAI, Keyword Research, Humanizer, Human Writer, Long Form Content
...
class AI_Scribe {
	public function __construct() {
		...
		add_action('wp_ajax_update_style_tone', [$this, 'update_style_tone']); // Register the new method
	...
	function update_style_tone() {
	    ...
	    if (!$contentSettings) {
	        error_log('Content settings not found.');
	    ...
	    if ($updated === false) {
	        error_log('Failed to update content settings.');
	...
	*/
	public function activate() {
	    ...
	    // Set the default language array if it doesn't exist
	    //if (!get_option('ai_scribe_languages')) {
        //    update_option('ai_scribe_languages', $default_lang_array);
        //}
	...
	*/
	public function content_data() {
		...
		// Check if a new language is submitted
		if (!empty($_POST['custom_language'])) {
		    $new_language = sanitize_text_field($_POST['custom_language']);

		    $lang = $new_language;

		    // Retrieve the current array of languages
		    $languages = get_option('ai_scribe_languages');
		    if (!is_array($languages)) {
		        $languages = []; // Initialize as an array if not set
		    }

		    // Check if the language isn't already in the array
		...
		if (isset($_POST['mode'])) {
		    ...
		    // If 'mode' is not set in POST, retrieve the current value
		    $existingSettings = get_option('ab_gpt3_content_settings');
		    $mode = $existingSettings['mode'] ?? 'standard';
		}
		...
		$savedSettings = get_option('ab_gpt3_content_settings');
		error_log('Saved mode: ' . $savedSettings['mode']);
		error_log('Saved settings: ' . print_r($frmArr, true));


		// Handle prompts
		$existingPromptsContent = get_option('ab_prompts_content') ?: [];

		// Get new prompts content from form input
		$newPromptsContent = array_map('sanitize_textarea_field', $_POST['prompts_content'] ?? []);

		// Save user's custom instructions separately
		$customInstructions = $newPromptsContent['instructions_prompts'] ?? '';

		// Remove 'instructions_prompts' from $newPromptsContent before merging
		unset($newPromptsContent['instructions_prompts']);


		// Only save the custom instructions if 'standard' mode is selected
		if ($mode == 'standard') {
		    $promptsContent['user_instructions'] = $customInstructions;
		} else {
		    // In other modes, append specific instructions as needed
		    $promptsContent['user_instructions'] = $instructions;
		}

		
		// Merge existing prompts content with new prompts content
		$promptsContent = array_merge($existingPromptsContent, $newPromptsContent);

		// Update the option with all prompts
