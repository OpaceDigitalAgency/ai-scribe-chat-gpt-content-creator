...
Does the article provide an original, interesting and engaging perspective on the topic?',
    ...
    public function generate_gpt_image_1(
        ...
        if (is_array($debug_messages)) {
            ...
            $debug_messages[] = "🎨 IMPORTANT: Using gpt-image-1 model (NOT dall-e-3)";
        ...
        if (!empty($isError)) {
            ...
            if ($actionInput == "article") {
                ...
                if (!empty($title)) {
                    ...
                    $title = preg_replace('/\.+\s*$/', '', $title); // Remove trailing dots
                    $title = trim($title);
                    $debug_messages[] = "🧹 Title cleaned (removed bullets and trailing punctuation): " . $title;
                }

                // Create image prompt with style setting
                $image_style = get_option('ab_image_style', 'Photorealistic');
                $image_prompt = !empty($title)
                    ? $title . " - Create an image based on this title in the style of " . $image_style . ". You must not include any text, characters, symbols, or writing. Highly detailed and stylised to match the title."
                    : "Create a default watermark type image in the style of " . $image_style;
                
                $debug_messages[] = "🎨 Image prompt created: " . $image_prompt;
                $debug_messages[] = "🎨 Image style setting: " . $image_style;
                $debug_messages[] = "🎨 Using GPT-image-1 model (NOT DALL-E)";
                ...
                if (isset($image_url) && !empty($image_url)) {
                    ...
                    if ($temp_file && !is_wp_error($temp_file)) {
                        ...
                        $attachment_id = media_handle_sideload(
                            $file_array,
                            0,
                            $title // Use clean article title as description
                        );
                        ...
                        if (is_wp_error($attachment_id)) {
                            ...
                            $attachment_data = [
                                'ID' => $attachment_id,
                                'post_title' => $title, // Image title (not slug) - clean article title
                                'post_excerpt' => $title, // Image caption - article title
                            ];
                            wp_update_post($attachment_data);
                            ...
                            $debug_messages[] = "✅ Image metadata updated - Title: '$title', Caption: '$title', Alt: '$title'";
                            
                            // Get the attachment URL after upload
                            $attachment_url = wp_get_attachment_url($attachment_id);
                            $debug_messages[] = "✅ Image successfully uploaded to media library: " . $attachment_url;

                            // Generate image HTML with proper alt, title, and caption
                            $image_html = '<figure class="wp-block-image">' .
                                '<img src="' .
                                esc_url($attachment_url) .
                                '" alt="' .
                                ...
                                esc_attr($title) .
                                '" class="wp-image-' . $attachment_id . '" />' .
                                '<figcaption>' . esc_html($title) . '</figcaption>' .
                                '</figure>';
