...
<div class="create_template_cont_sec">
	...
	<div class="ai-scribe-header">
		...
		<div class="cost-calculator">
			<div class="cost-heading" style="font-size: 12px; font-weight: 600; color: white; margin-bottom: 8px; text-align: center;">Cost Estimator</div>
			<div class="cost-display">
				...
				<div class="cost-item">
					...
					<span class="cost-value" id="actual-cost">$0.00</span>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="article-main">
		...
		<div class="article-right">
			...
			<div class="selected-model-section" style="background-color: #f8f9fa; border: 1px solid #dee2e6; border-radius: 5px; padding: 15px; margin-bottom: 20px;">
				<h2 style="margin-top: 0; color: #495057; font-size: 16px; font-weight: bold;">SELECTED MODEL</h2>
				<?php
				$ai_engine_settings = get_option('ab_gpt_ai_engine_settings', []);
				$selected_model = $ai_engine_settings['model'] ?? 'gpt-4o';
				$openai_key = $ai_engine_settings['api_key'] ?? '';
				$anthropic_key = $ai_engine_settings['anthropic_api_key'] ?? '';
				$image_generation_enabled = get_option('ab_enable_image_generation', 1) == 1;

				// Determine provider
				$is_anthropic = strpos($selected_model, 'claude') !== false;
				$provider = $is_anthropic ? 'Anthropic' : 'OpenAI';

				// Format model name for display
				$display_model = ucfirst(str_replace(['-', '_'], [' ', ' '], $selected_model));

				// Create compact display line
				$image_status = $image_generation_enabled ? '✓ Enabled' : '❌ Disabled';
				$image_color = $image_generation_enabled ? '#28a745' : '#dc3545';
				?>
				<div style="font-size: 14px; line-height: 1.6;">
					<p style="margin: 5px 0;">
						<strong>Provider:</strong> <?php echo esc_html($provider); ?> |
						<strong>Model:</strong> <?php echo esc_html($display_model); ?> |
						<strong>Images:</strong> <span style="color: <?php echo $image_color; ?>"><?php echo $image_status; ?></span>
					</p>
				</div>
			</div>
			...
			<div class="option-prompt">
				...
				<P><strong>New in v2.6:</strong> Now with GPT-4.5, OpenAI o3 & Claude Sonnet 4/Opus 4 support, dual API integration, smarter GPT-4o image generation, and live token cost estimates.</P>

				<P><strong>Other v2.0 Updates</strong><br>
				• Humanizer Modes: Add natural or witty personality to your content<br>
				• DALL-E 3 image integration</P>
			</span>
